...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	fmt.Println("\nStart Time for AgeAnalysis Activity: ", tStart)
	input := &Input{}
	...
	if demoID != lastDemoID {
		fmt.Println(fmt.Sprintf("\nStart preparing to switch models for AgeAnalysis Activity..."))
		tsStart := time.Now().UnixNano()
		fmt.Println("\nSwitching model for AgeAnalysis Activity starts: ", tsStart)
		...
		fmt.Println("\nSwitching model for AgeAnalysis Activity ends: ", tsEnd)
	}
			...
			for faceIndex := 0; faceIndex < len(imgjson.Bboxes); faceIndex++ {
				...
				if preds, ok := result[0].Value().([][]float32); ok {
					...
					if demoID < 3 {
						...
						age = strconv.Itoa(maxValueIndex)
					}
	...
	fmt.Println("\nEnd Time for AgeAnalysis Activity: ", tEnd)
	fmt.Println("\nThe Time Consumption for AgeAnalysis Activity: ", tEnd-tStart)
